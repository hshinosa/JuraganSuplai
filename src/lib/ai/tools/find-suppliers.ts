...
export async function findSuppliers(input: FindSuppliersInput): Promise<string> {
  const { lat, lng, searchTerm, radiusKm = 10, maxResults = 5 } = input;
  ...
  const { data, error } = await (supabase.rpc as Function)('find_nearby_suppliers', {
    lat,
    lng,
  ...
  if (availableSuppliers.length === 0) {
    return JSON.stringify({
      success: true,
      found: 0,
      message: `Tidak ada supplier tersedia untuk "${searchTerm || 'produk'}" dalam radius ${radiusKm}km`,
      suppliers: [],
    });
  }
