...
export async function actionRequestItem(
  ...
  try {
    ...
    const orderData: OrderInsert = {
      ...
      delivery_location: `POINT(${input.deliveryLng} ${input.deliveryLat})` as unknown as string,
    ...
    if (orderError || !order) {
      console.error('[actionRequestItem] Order creation failed:', orderError);
      return {
        success: false,
...
export async function actionCancelOrder(
  ...
  try {
    ...
    if (!['searching_supplier', 'waiting_buyer_approval', 'negotiating_courier', 'waiting_payment'].includes(order.status)) {
      return { success: false, message: 'Pesanan sudah tidak bisa dibatalkan.' };
    }
    ...
    if (order.supplier_id && order.supplier) {
      const supplierData = order.supplier as { phone: string; name: string };
      await sendWhatsApp({
        phone: supplierData.phone,
        message: `Maaf Pak ${supplierData.name}, order #${orderId.substring(0, 8)} baru saja dibatalkan pembeli.`,
      });
    }
...
export async function actionSimulatePayment(
  ...
  try {
    ...
    if (order.status !== 'waiting_payment') {
      return { success: false, message: 'Status pesanan tidak valid untuk pembayaran.' };
    }
    
    const supplierData = order.supplier as { id: string; phone: string };
    const totalAmount = order.buyer_price + order.service_fee + order.shipping_cost;
    const supplierAmount = order.buyer_price; // Supplier gets product price only
    
    // Process payment via RPC
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error: paymentError } = await (supabase.rpc as Function)('process_payment_escrow', {
      p_order_id: orderId,
    ...
    if (paymentError) {
      console.error('[actionSimulatePayment] Payment error:', paymentError);
    ...
    await sendWhatsApp({
      ...
      message: templates.supplierPaymentReceived({
        ...
        courier_name: order.courier_id ? 'Kurir Aktif' : 'Menunggu kurir',
        courier_phone: '-',
      }),
    });
    ...
    return { success: true, message: 'Pembayaran berhasil! Supplier sedang mempersiapkan barang.' };
 ...
 */
export async function actionConfirmDelivery(
  input: { orderId: string; buyerId: string; confirmationCode?: string }
): Promise<{ success: boolean; message: string }> {
  const supabase = createAdminClient();
  const { orderId } = input;
  ...
  try {
    ...
    if (order.status !== 'delivered' && order.status !== 'shipping') {
      return { success: false, message: 'Pesanan belum dalam status pengiriman.' };
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.rpc as Function)('complete_order_and_release_escrow', {
      p_order_id: orderId,
      p_supplier_id: supplierData.id,
    ...
    return { success: true, message: 'Pesanan selesai! Terima kasih telah menggunakan JuraganSuplai.' };
 ...
 */
export async function actionReportDispute(
  input: { orderId: string; buyerId: string; reason: string; imageUrl?: string }
): Promise<{ success: boolean; message: string }> {
  const supabase = createAdminClient();
  ...
  try {
    ...
    return { 
      success: true, 
      message: 'Laporan dispute terkirim. Tim kami akan meninjau dalam 24 jam.' 
    };
